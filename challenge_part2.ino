...
void loop() {
  if(obstacleAhead()) avoidObstacle();
  pathScan();
  if (leftpath && !rightpath){
    if (boxStatus == "search" || seen_obstacles >= 2) {
      forward(50);
    } else {
      forward(250);
    }
  ...
  // on the left side of the line (not supposed to happen)
  else if(!leftpath && rightpath){
    backward(200);
  } 
...
void pathScan(){
  ...
  leftpath = (colorCheck() == "red" || colorCheck() == "black");
  turnRight(turnAmount);
  rightpath = (colorCheck() == "red" || colorCheck() == "black");  
...
void moveArmSmooth(int target) {
  ...
  const int UP_DELAY_MS = 25;    // slower up
  const int DOWN_DELAY_MS = 10;  // keep down faster
  const int STEP = 1;            // 1 degree per step
